<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<input v-model="searchForm.shangbaobiaoti" type="text" placeholder="上报标题" ></input>
			</view>
                                                      			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.fengxiandiqu" type="text" placeholder="风险地区" ></input>
			</view>
                              			<view v-if="queryIndex==2" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.shangbaoriqi" type="text" placeholder="上报日期" ></input>
			</view>
        ...
        <view @tap="onDetailTap(product)" class="uni-product" :style='{"padding":"0px 20rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 1% 20rpx 2%","borderColor":"rgba(130, 163, 157, 1)","backgroundColor":"#fff","borderRadius":"20rpx","borderWidth":"2rpx","width":"47%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
                                <view class="uni-product-title" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","fontSize":"28rpx","lineHeight":"56rpx","borderStyle":"solid"}'>{{product.shangbaobiaoti}}</view>
                              			<image :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","borderRadius":"8rpx","borderWidth":"0","width":"95%","borderStyle":"solid","height":"260rpx"}' mode="aspectFill" class="uni-product-image" :src="product.shangbaotupian?baseUrl+product.shangbaotupian.split(',')[0]:''"></image>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.shangbaobiaoti){
					params['shangbaobiaoti'] = '%' + this.searchForm.shangbaobiaoti + '%'
				}
				if(this.searchForm.fengxiandiqu){
					params['fengxiandiqu'] = '%' + this.searchForm.fengxiandiqu + '%'
				}
				if(this.searchForm.shangbaoriqi){
					params['shangbaoriqi'] = '%' + this.searchForm.shangbaoriqi + '%'
				}
			...
			async search(){
				...
				if(this.searchForm.shangbaobiaoti){
					searchForm['shangbaobiaoti'] = '%' + this.searchForm.shangbaobiaoti + '%'
				}
				if(this.searchForm.fengxiandiqu){
					searchForm['fengxiandiqu'] = '%' + this.searchForm.fengxiandiqu + '%'
				}
				if(this.searchForm.shangbaoriqi){
					searchForm['shangbaoriqi'] = '%' + this.searchForm.shangbaoriqi + '%'
				}
                let res = {};
                if(this.userid) {
