      ...
      >
        ...
        <el-form-item label="健康码">
          <file-upload
            ...
            :limit="1"
            :multiple="false"
            :fileUrls="dialogForm.healthImg ? dialogForm.healthImg : ''"
...
export default {
  ...
  methods: {
    ...
    imgUploadChange(fileUrls) {
      this.dialogForm.healthImg = fileUrls;
    ...
    confirmAreaFormHandle() {
      if (this.dialogForm.healthImg != null) {
        this.dialogForm.healthImg = this.dialogForm.healthImg.replace(
        ...
        if (valid) {
          that
            .$http({
              url:
                that.dialogitle == "添加"
                  ? "/foreign_personnel/save"
                  : "/foreign_personnel/update",
              method: "post",
              data,
            })
            .then(({ data: res }) => {
    ...
    searchDatePickeChange() {
        this.searchForm.startTime =  this.searchDateRange[0]
        this.searchForm.endTime = this.searchDateRange[1] 
    },
    formDatePickerChange() {
        this.dialogForm.startTime = this.formDateRange[0]
        this.dialogForm.endTime = this.formDateRange[1]
    }
  },
};
</script>
