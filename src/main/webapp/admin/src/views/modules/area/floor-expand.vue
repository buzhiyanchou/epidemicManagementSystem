<template>
  <div class="floor-expand-container" v-if="isShow">
    <el-form
      :inline="true"
      :model="searchForm"
      class="form-content"
    ...
    <div class="table-content">
        ...
        <el-table-column type="expand">
          <template slot-scope="{row}">
            <RoomExpand :floorInfo="row" :userInfo="userInfo"/>
          </template>
        </el-table-column>
          <el-table-column
            type="selection"
    ...
    >
      <el-form
        ...
        label-width="100px"
      >
       <el-form-item label="楼栋名称/号" prop="floorNum">
          <el-input
...
<script>
import RoomExpand from './room-expand.vue'
export default {
  props: {
    areaInfo: null,
    userInfo: null,
  ...
  methods: {
    ...
    confirmFormHandle() {
      ...
      this.$refs['ruleForm'].validate((valid) => {
          if (valid) {
            that.$http({
              url: that.dialogTitle == '添加'? '/floor/save':'/floor/update',
              method: 'post',
              data
            }).then(({ data: res }) =>{
